...
\noindent
Our Monte Carlo simulation seems to show that the probability density is mostly around the smaller numbers, it\keyword{conjecture} also looks as if the probability density function is decreasing as k increases. It also reinforces our idea that F can only ever be even or odd, as the distribution only contains either even or odd values.

\begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{hatmontecarlo.png}
    \captionof{figure}{Monte Carlo for $N = 10,50,500,2026$ with 10000 simulations}
    \label{fig:placeholder}
\end{figure}
We can see from our montecarlo that the distribution seems to be quite inverse exponential looking, i wonder if we can find a good approximation of the graph by finding suitable $a,b,c$ to fit $ae^{-bx} +c$ to our data. With small enough error this would be reliable.
...
\begin{claim}For each n, F can only take either even or odd values
...
\begin{claim}
    Once the largest number leaves the hat, it can no longer be readded
\end{claim}
...
\begin{claim}We can approximate $\mathbb{P}(F < K)$ using an exponential approximation:


\end{claim}
whilst i am almost certain this is true, i could find no way of proving it. By direct computation i have shown it true up to n = 14
\begin{claim}There are $\sim4\pi\left(\frac{n^2}{2e^2}\right)^{n}$ move sequences, $\mathbb{P}(F = k)$
...
\[
P(n)\;:=\prod^{n}_{k=2}{k\choose2} = \prod^{n}_{k=2}\frac{k(k-1)}{2} = \frac{1}{2^{n-1}}\prod^{n}_{k=2}k\prod^{n}_{k=2}(k-1) =\frac{n!(n-1)!}{2^{n-1}}
\] 
...
\]
Apply this formula with $m=n$ and for $m=n-1$:
\[
P(n)
=\frac{1}{2^{n-1}}\sqrt{2\pi n}\left(\frac{n}{e}\right)^{n}
\sqrt{2\pi n}\left(\frac{n-1}{e}\right)^{n-1}
\exp\!\left(\frac{\theta_{n}}{12n}+\frac{\theta_{n-1}}{12(n-1)}\right).
\]
Rearrange:
\[
P(n)
=(2\pi)\,\frac{\sqrt{n(n-1)}}{2^{n-1}}\,
\frac{n^{n}(n-1)^{n-1}}{e^{2n-1}}\,
\exp\!\left(\frac{\theta_{n}}{12n}+\frac{\theta_{n}}{12(n-1)}\right).
\]
Equivalently,
\[
P(n)
=4\pi\left(\frac{n^2}{2e^2}\right)^{n}\,R_n,\quad
R_n
:=e\,
\left(1-\frac{1}{n}\right)^{n-{1/2}}
\exp\!\left(\frac{\theta_{n}}{12n}+\frac{\theta_{n-1}}{12(n-1)}\right).
\]
As $n\to\infty$, we have $R_n\to 1$, hence
\[
P(n)\sim 4\pi\left(\frac{n^2}{2e^2}\right)^{n}.
\]
Many of these different sets calculated will be identical but found with different choices of (a,b), but a strong dependence on the previous move means we cannot take any shortcuts, making this incredibly difficult to calculate, with this difficulty increasing super exponentially as n increases.
...
\section{Review}
I have given many reasons as \keyword{check}for why this question would be hard to solve, demonstrating why properties of F are so elusive, as other then the trivial properties of F, eg its a non zero integer between 0 and 2026, its hard to show much more concusivly

This was a far less analytical assignment then the first two, with large amounts of computation required instead of elegant proofs. This is where python was exceptionally useful as for anything above N = 4, it would be unreasonable to do any sort of calculations by hand. My attack for the question was mainly to find interesting ways of representing the question through python.
...
F_n=\sum_{k=1}^n k\,\varepsilon_k,\qquad \varepsilon_k\in\{+1,-1\}.
\end{equation}
...
\noindent
\begin{minipage}[t]{0.52\textwidth}
Why does fig 6 looks Gaussian? Writing
$P=\{k:\varepsilon_k=+1\}$ and $M=|P|$, we have
\[
F_n=\sum_{k\in P}k-\sum_{k\notin P}k \;=\;2\sum_{k\in P}k-\sum_{k=1}^n k
\;=\;2T_M-S,
\]
where $S=\sum_{k=1}^n k$ and $T_M:=\sum_{k\in P}k$. By exchangeability, conditional on $M=m$, $P$ is uniform over all $m$-subsets of $\{1,\dots,n\}$, so $T_m$ is the sum of a simple
random sample without replacement from $\{1,\dots,n\}$. Such sample sums satisfy the finite-population central limit theorem for $m\asymp n$, so $T_m$ (hence $F_n$) is approximately normal. This produces the observed bell-shaped distribution, even though $\varepsilon_k$ are dependent.
...
\captionof{figure}{Distribution of $F_{50}$, $1\times 10^6$ Trials}
\end{minipage}
\vspace{5pt}

We can see that simply introducing linearity into our move for  picking values makes the question far easier to tackle

\section*{Code}
The code for this assignment can be found on my GitHub page:\\  
\url{https://github.com/siriwarwick/book/tree/main/Chapter4}

In particular, \texttt{hat.ipynb} was used to plot figures 3,4,5 and 6 as well as for the calculations in Claim 3. Figures 1 and 2 were plotted with Tikz.
